...
function calculate() {
    ...
    const roiRate = parseFloat(document.getElementById('roi').value) / 100;
    ...
    // Projection runs until age 99 instead of capping at 30 years
    const maxAge = 99;
    ...
    for (let i = 0; i < yearsToProject; i++) {
        ...
        // Step F: Apply Flow to Balance
        currentBalance += nettPremium; // Add premium
        let totalDeductions = basicCOI + actualMultiCICost + actualMhseCost + adminCharge;
        currentBalance -= totalDeductions; // Subtract charges
        
        // Step G: Apply Investment ROI (Only if balance is positive)
        if (currentBalance > 0) {
            currentBalance += (currentBalance * roiRate);
        }
        ...
        let basicCOIString = basicCOI === 0 ? '<span class="text-slate-300">-</span>' : `- ${formatCurrency(basicCOI)}`;
